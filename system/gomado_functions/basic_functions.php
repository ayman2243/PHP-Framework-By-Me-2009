<?php
function addUrlGetValue($url,$get_var_name,$get_value)
{
	$out1 = parse_url($url, PHP_URL_QUERY);
	
	$url = str_replace(array($out1,'?'),'',$url);
	
	parse_str($out1,$output);
	...
	return $url."?".http_build_query($output).$xChar.$get_var_name."=".$get_value;
...
function striptag($string, array $tags){
	...
	foreach($tags as $v){
		...
		$check_split = explode($v.'>',$string);
		
		if(count($check_split) == 3){
			$search[] =  '&lt;'.$v.'&gt;';
			$search[] =  '&lt;/'.$v.'&gt;';
	...
	return str_replace($search,$replace,htmlentities($string));
}

################################################################################################################

function highlight($word, $subject,$opraitor = " ") {
    $split_subject = explode(" ", strtolower($subject));

    $split_word = explode($opraitor, strtolower($word));

    foreach ($split_subject as $k => $v){
      	 foreach ($split_word as $k2 => $v2){
	      	 if($v2 == $v && $v2 != NULL){
	      	 	
	      	 	$split_subject[$k] = "<span style='color:#F60' class='highlight'>".$v."</span>";
...
function verify_email_and_link_in_post($c){
	 ...
	 $c=ereg_replace("[-a-z0-9!#$%&\'*+/=?^_`{|}~]+@([.]?[a-zA-Z0-9_/-])*",".
	   ".$e,$c);
	
	 $c=ereg_replace("@", " at ",$c);
	 
	 $c=eregi_replace('(<a [^<]*href=["|\']?([^ "\']*)["|\']?[^>].*>([^<]*)</a>)',".
	   ".$l,$c);
	 
	 $c=ereg_replace("[a-zA-Z]+://([.]?[a-zA-Z0-9_/-])*",".
	   ".$l,$c);
	 
	 $c=ereg_replace("(^| )(www([-]*[.]?[a-zA-Z0-9_/-?&%])*)",".
	   ".$l,$c);
	 
	 $c=ereg_replace("[-a-z0-9!#$%&\'*+/=?^_`{|}~]+[.]+[-a-z0-9]",".
	   ".$l,$c);
	 return $c;
}
####################################################################################################################
...
function Cmail()
{
	require 'system/lib/php/email/class.phpmailer.php';
...
function fix_tag($string, array $allowed_tags){
	...
	foreach($allowed_tags  as $tag):
	    ...
	    if($tag != "a" && $tag != "http"){
          ...
          if (preg_match_all("/(&lt;".$tag."&gt;)(.*?)(&lt;\/".$tag."&gt;)/", $string,$match)) {
				...
				$string = str_replace(array("&lt;".$tag."&gt;","&lt;/".$tag."&gt;"),array("[]","[/]"),$string);
	
				//print $str."<br>";
	
				foreach($match[2] as $k => $v):
		
				$string = str_replace("[]".$v."[/]","<".$tag.">".$v."</".$tag.">",$string);
		   ...
		   if (preg_match_all("/&lt;a.*? href=[\"|'](http:\/\/.*?)[\"|'].*?&gt;(.*?)(&lt;\/a\s*&gt;)/", str_replace(array('&quot;'),array('"'),$string),$match))              {
    
           		foreach($match[1] as $key => $value):
				   $string =  str_replace(array('&quot;'),array('"'),$string);
				   $string = str_replace($match[0][$key],"<a href=\"".trim($value)."\">".$match[2][$key]."</a>",$string);
		  ...
		  if (preg_match_all("/http:\/\/(www\.|WWW\.|)([a-zA-Z0-9\-]+\.|)([a-zA-Z0-9\-]{1,60})\.([a-zA-Z]{1,6})(\/[a-zA-Z0-9\/\-\+\_\%\.\?\&amp;\&\=]+|)\s{1,1}/", $string,$match)) {
                
				//print_r($match);
				
				foreach($match[0] as $v):
				
				   $string = str_replace($v,"<a href=\"".trim($v)."\">".trim($v)."</a>",$string);
